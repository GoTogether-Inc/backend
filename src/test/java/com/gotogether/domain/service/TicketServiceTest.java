...
class TicketServiceTest {
	...
	void createTicket_Success() {
		...
		assertThat(result.getDescription()).isEqualTo(request.getTicketDescription());
		assertThat(result.getPrice()).isEqualTo(request.getTicketPrice());
		assertThat(result.getAvailableQuantity()).isEqualTo(request.getAvailableQuantity());
		assertThat(result.getType()).isEqualTo(request.getTicketType());
		assertThat(result.getStatus()).isEqualTo(TicketStatus.OPEN);

		verify(eventFacade).getEventById(request.getEventId());
	...
	void getTickets_Success() {
		...
		assertThat(result.get(0).getTicketDescription()).isEqualTo(ticket.getDescription());
		assertThat(result.get(0).getTicketPrice()).isEqualTo(ticket.getPrice());
		assertThat(result.get(0).getAvailableQuantity()).isEqualTo(ticket.getAvailableQuantity());
	...
	void updateTicketStatusToCompleted_Success() {
		// GIVEN
		when(ticketRepository.findById(any())).thenReturn(Optional.of(ticket));
